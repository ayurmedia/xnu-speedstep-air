...
static int iess_handle_usage SYSCTL_HANDLER_ARGS
{
	...
	if (!req->newptr) { // reading
		...
		for (int i = NumberOfPStates - 1; i >= 0; i--) {
			...
			snprintf((frequencyUsage + curpos), 10, "%llu.%llu%%     ", pc / 100, pc % 100);
			curpos += 8;
		}
...
char* getFreqList() {
	...
	char* sfreqs = new char[5 * NumberOfPStates];
	int c1 = 0;
	for (int i = NumberOfPStates-1; i >= 0; i--) {
 		snprintf((sfreqs + c1), 6, "%d ", PStates[i].AcpiFreq);
...
int FindClosestPState(int wantedFreq) {
  ...
  dbg( "i: %d diff: %d check: %d best: 0 \n", 0, bestdiff, PStates[0].AcpiFreq );
    ...
    dbg( "i: %d diff: %d check: %d best: %d \n", i, bestdiff, PStates[i].AcpiFreq, bestpstate );
